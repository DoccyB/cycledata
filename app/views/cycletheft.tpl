...
{* FILTERS *}
	<form action="{$currentUrl}" class='filter'>
		...
		<p>Police Force:</p>
		<select name="force">
			{foreach $distinctValues.reportedBy as $reportedBy}
			<option value="{$reportedBy|lower|strip:''}">{$reportedBy}</option>
			{/foreach}
		</select><br>
		<p>Status:</p>
		<select name="status">
			{foreach $distinctValues.status as $status}
			<option value="{$status|lower|strip:''}">{$status}</option>
			{/foreach}
		</select> <br>
		<p>Month</p>
		<select name="month">
			{foreach $distinctValues.date as $date}
